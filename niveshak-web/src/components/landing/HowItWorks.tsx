...
export function HowItWorks() {
  ...
  return (
    <section className="px-6 py-8">
      <p className="text-[10px] font-semibold text-lime uppercase tracking-widest mb-6">
        {t('landing.howItWorks.label')}
      </p>
      <div className="flex flex-col gap-6">
        {STEPS.map(({ num, titleKey, descKey }) => (
          <div key={num} className="flex gap-4 items-start">
            <div className="w-8 h-8 rounded-sm bg-lime flex items-center justify-center flex-shrink-0">
              <span className="font-sora font-bold text-black text-xs">{num}</span>
            </div>
            <div>
              <p className="font-sora font-semibold text-white text-sm mb-1">
                {t(titleKey)}
              </p>
