...
export function AIExplanationCard() {
  ...
  const doFetch = useCallback(
    async (signal: AbortSignal) => {
      ...
      setDisplayedText('');
      try {
        const payload = buildExplanationPayload(results, answers, language as 'en' | 'hi');
  ...
  return (
    <div className="bg-navy-pale border border-navy/20 rounded-lg p-5">
      ...
      {state === 'done' && (
        <div>
          <p className="text-content text-sm leading-relaxed">{displayedText}</p>
          <button
            type="button"
      ...
      {state === 'error' && (
        <div>
          <p className="text-error text-xs mb-2 break-words">{errorMsg}</p>
          <button
            type="button"
            onClick={handleRegenerate}
