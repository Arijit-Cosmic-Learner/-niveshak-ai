    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {items.map(item => (
          <div
            key={item.label}
            className="bg-black-light border border-border-dark rounded-lg p-4 flex flex-col gap-1"
          >
            <p className="font-sora font-extrabold text-lime text-2xl">{item.value}</p>
            <p className="text-grey-mid text-xs">{item.label}</p>
          </div>
        ))}
      </div>
    </div>
