import { useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuthStore } from '@store/useAuthStore';
import { supabase } from '@lib/supabase';
import { LogoMark } from '@components/common/LogoMark';

const REDIRECT_URL = `${window.location.origin}/auth/callback`;

async function signInWithGoogle() {
  if (!supabase) return;
  await supabase.auth.signInWithOAuth({
    provider: 'google',
    options: { redirectTo: REDIRECT_URL },
  });
}

export default function SplashPage() {
  const navigate = useNavigate();
  const user = useAuthStore(s => s.user);
  const isLoading = useAuthStore(s => s.isLoading);

  // Already signed in — skip splash and go straight to the app
  useEffect(() => {
    if (!isLoading && user) {
      navigate('/home', { replace: true });
    }
  }, [isLoading, user, navigate]);

  return (
    <div className="min-h-screen bg-bg flex flex-col items-center justify-center px-6 gap-10">
      {/* Logo + wordmark */}
      <div className="flex flex-col items-center gap-4">
        <LogoMark size="lg" />
        <div className="text-center">
          <h1 className="font-sora font-extrabold text-content text-3xl tracking-tight">
            Niveshak.AI
          </h1>
          <p className="text-sub text-sm mt-2 max-w-xs leading-relaxed">
            India's first goal-matched investment discovery platform
          </p>
        </div>
      </div>

      {/* Google sign-in button */}
      <div className="flex flex-col items-center gap-4 w-full max-w-xs">
        <button
          onClick={signInWithGoogle}
          className="w-full flex items-center justify-center gap-3 px-6 py-3.5 rounded-xl bg-card border border-line hover:border-accent/50 hover:shadow-md transition-all"
        >
          {/* Google G */}
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
            <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
            <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
            <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05"/>
            <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
          </svg>
          <span className="font-sora font-semibold text-content text-sm">
            Continue with Google
          </span>
        </button>

        <p className="text-hint text-[11px] text-center leading-relaxed">
          By continuing, you agree to our terms of service.<br />
          Your data is never sold.
        </p>
      </div>
    </div>
  );
}
