...
function buildPrompt(p: RequestPayload): string {
  const instrLines = p.availableInstruments.map((inst, i) =>
    `${i + 1}. id="${inst.id}" | ${inst.nameEn} | ${inst.type} | Returns: ${inst.returnsLabel} | Risk: ${inst.riskLevel} | Tax: ${inst.taxBenefit ?? 'None'} | Lock-in: ${inst.lockInYears ? inst.lockInYears + ' yr' : 'None'} | Min \u20B9${inst.minMonthly}/mo | Govt-backed: ${inst.isGovernmentBacked ? 'Yes' : 'No'} | Why: ${inst.whyEn}`
  ).join('\n');
...
- Monthly income: \u20B9${p.inHandIncome.toLocaleString('en-IN')} | Monthly surplus: \u20B9${p.monthlySurplus.toLocaleString('en-IN')}
...
- Corpus needed: \u20B9${p.adjustedCorpus.toLocaleString('en-IN')} | Monthly investment budget: \u20B9${p.monthlyInvestment.toLocaleString('en-IN')}
